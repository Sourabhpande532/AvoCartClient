...
export const Checkout = () => {
    ...
    useEffect( () => { if ( addresses && addresses.length && !selectedAddressId ) setSelectedAddressId( addresses[0]._id ); }, 
    ...
    const doCheckout = async () => {
        ...
        const order = await placeOrders( { items: cart.map( ci => ( { product: ci.product._id, qty: ci.qty } ) ), total: finalTotal, address: selectedAddress } );
        if ( order ) {
            navigate( '/profile' );
        }
    };
    ...
    return (
        <div className="row">
            <div className="col-md-8">
                ...
                { addresses.length === 0 && <p>No addresses found. Add from profile page.</p> }
                { addresses.map( a => (
                    <div key={ a._id } className={ `card mb-2 p-2 ${ selectedAddressId === a._id ? 'border-primary' : '' }` } onClick={ () => setSelectedAddressId( a._id ) } style={ { cursor: 'pointer' } }>
            ...
            <div className="col-md-4">
                <div className="card p-3">
                    ...
                    <button className="btn btn-success" onClick={ doCheckout }>Place Order</button>
                </div>
            </div>
        </div>
    )
